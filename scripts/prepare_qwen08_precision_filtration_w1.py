"""Seal an analysis-only authorization for the precision filtration/w1 run."""
...
def main(args: argparse.Namespace) -> None:
    ...
    sources = {
        ...
        "analysis_module": ROOT / "rsi_topology" / "qwen_precision_filtration_w1.py",
        "analysis_entrypoint": ROOT / "scripts" / "run_qwen08_precision_filtration_w1.py",
    ...
    command = [
        ...
        "--parent-protocol", str(args.parent_protocol.resolve()),
        "--manifest", str(args.manifest.resolve()),
        "--parent-result", str(args.parent_result.resolve()),
        "--parent-run-receipt", str(args.parent_run_receipt.resolve()),
    ...
    value.update(
        {
            ...
            "parent_scientific_protocol": {"path": str(args.parent_protocol.resolve()), "sha256": sha256_file(args.parent_protocol)},
            "parent_analysis_result": {"path": str(args.parent_result.resolve()), "sha256": sha256_file(args.parent_result)},
            "parent_run_receipt": {"path": str(args.parent_run_receipt.resolve()), "sha256": sha256_file(args.parent_run_receipt)},
    ...
    print(json.dumps({"status": "sealed", "run_id": value["run_id"], "implementation_commit": commit, "authorization_sha256": sha256_file(args.output)}, indent=2))
