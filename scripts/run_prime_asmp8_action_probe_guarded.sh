...
RUN_ROOT="${RUN_ROOT:-/workspace/asmp8_action_probe_v0_1}"
OUTPUT_DIR="${OUTPUT_DIR:-${RUN_ROOT}/output}"
UNIT_NAME="${UNIT_NAME:-asmp8-action-probe-v0-1}"
...
RUNNER=(
    /usr/bin/python3 "${RUN_ROOT}/scripts/run_qwen08_action_probe.py"
    --run-id asmp8-qwen08-action-probe-prime-spot-v0-1-20260725
    --server-path /workspace/llama.cpp/build-sm80/bin/llama-server
    --model-path "${RUN_ROOT}/model/Qwen3.5-0.8B-Q4_K_M.gguf"
    --prompt-manifest "${RUN_ROOT}/input/action_probe_manifest.json"
