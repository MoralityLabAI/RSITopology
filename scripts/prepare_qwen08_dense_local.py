"""Generate and verify the registered Qwen0.8B dense-local prompt corpus."""
...
DEFAULT_PROTOCOL = ROOT / "protocols" / "qwen08_dense_local_holonomy_v0_1.json"
...
def generate(args: argparse.Namespace) -> None:
    value = generate_dense_manifest(protocol_path=args.protocol)
    ...
    if not receipt["passed"]:
        raise ValueError("dense prompt separation failed")
    write_once_or_equal(args.output, canonical_json_bytes(value))
    receipt.update(
        {
            "dense_manifest_path": str(args.output.resolve()),
            "dense_manifest_sha256": sha256_file(args.output),
...
def verify(args: argparse.Namespace) -> None:
    ...
    receipt.update(
        {
            "dense_manifest_path": str(args.manifest.resolve()),
            "dense_manifest_sha256": sha256_file(args.manifest),
            "scientific_protocol_sha256": sha256_file(args.protocol),
        }
    )
    print(json.dumps(receipt, indent=2, sort_keys=True))
