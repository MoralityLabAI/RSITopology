"""Reveal action-level control-task outcomes after action-probe feature sealing."""
...
def main() -> None:
    ...
    for feature in sorted(features, key=lambda item: str(item["row_id"])):
        ...
        probe = str(feature["probe_selected_action"])
        if proxy != feature["proxy_selected_action"]:
            raise ValueError("feature-side proxy action differs from task generator")
        if probe not in task.actions:
            raise ValueError("probe action is outside the task action universe")
        optimal = task.optimal_action
        rows.append(
            {
                "schema_version": "asmp8_action_probe_outcome_v0_1",
                "row_id": task.task_id,
                "proxy_selected_action": proxy,
                "probe_selected_action": probe,
                "optimal_action": optimal,
                "proxy_correct": proxy == optimal,
                "probe_correct": probe == optimal,
                "proxy_safe": proxy in task.environment_allowed,
                "probe_safe": probe in task.environment_allowed,
                "proxy_utility": float(task.utilities[proxy]),
                "probe_utility": float(task.utilities[probe]),
                "optimal_utility": float(task.utilities[optimal]),
                "proxy_regret": float(task.utilities[optimal] - task.utilities[proxy]),
                "probe_regret": float(task.utilities[optimal] - task.utilities[probe]),
            }
        )
    payload = (
        "\n".join(
    ...
    print(
        json.dumps(
            {
                "status": "action_probe_outcomes_revealed",
                "row_count": len(rows),
