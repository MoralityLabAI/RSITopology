...
RUN_ROOT="${RUN_ROOT:-/workspace/asmp9_measurement_v035}"
REPO_ROOT="${REPO_ROOT:-${RUN_ROOT}/repo}"
OUTPUT_DIR="${OUTPUT_DIR:-${RUN_ROOT}/output}"
REGISTRATION="${REGISTRATION:-${REPO_ROOT}/ultra-experiments/millennium/asmp9_reward_gauge_census/measurement_channel_v0_35/measurement_pilot_registration_v035.json}"
UNIT_NAME="${UNIT_NAME:-asmp9-measurement-v035}"
readonly GPU_MEMORY_LIMIT_MB=1600
readonly GPU_TEMPERATURE_LIMIT_C=88
readonly GPU_CLEAN_START_CEILING_MB=64
...
if [[ ! "${BASELINE_GPU_MB}" =~ ^[0-9]+$ ]]; then
  echo "GPU baseline telemetry unavailable" >&2
  exit 3
fi
...
RUNNER=(
  /usr/bin/python3
  "${REPO_ROOT}/ultra-experiments/millennium/asmp9_reward_gauge_census/measurement_channel_v0_35/run_measurement_pilot_v035.py"
...
while kill -0 "${SUPERVISOR_PID}" 2>/dev/null; do
  ...
  if [[ -z "${SAMPLE}" ]]; then
    ABORT_REASON="gpu_telemetry_unavailable"
...
payload = {
    "schema_version": "asmp9_measurement_prime_wrapper_summary_v0_35",
