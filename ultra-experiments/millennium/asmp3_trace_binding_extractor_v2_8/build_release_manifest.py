...
MANIFEST_PATH = HERE / "RELEASE_MANIFEST_v2_8.json"
RELEASE_FILES = (
    ...
    ".gitignore",
    "README.md",
    "TRACE_BINDING_EXTRACTOR_THEOREM_v2_8.md",
    "COMPLETION_AUDIT_v2_8.md",
    "trace_binding_extractor.py",
    "run_trace_binding_extractor.py",
    "verify_trace_binding_extractor.py",
    "test_trace_binding_extractor.py",
...
def build_manifest():
    ...
    return {"schema_version":"asmp3_trace_binding_extractor_release_v2_8","experiment_id":"ASMP-3-TRACE-BINDING-EXTRACTOR-v2.8","release_status":"operational_refute_binding_compiler_and_finder_extractor","deterministic":True,"file_count":len(files),"files":files}


def write_manifest(): MANIFEST_PATH.write_text(json.dumps(build_manifest(),indent=2,sort_keys=True)+"\n",encoding="utf-8",newline="\n")


def verify_manifest(): return MANIFEST_PATH.is_file() and json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))==build_manifest()


def main():
    write_manifest()
    if not verify_manifest(): raise RuntimeError("ASMP-3 v2.8 manifest self-check failed")
    print(f"ASMP-3 v2.8 release manifest sealed: {len(RELEASE_FILES)} files")
