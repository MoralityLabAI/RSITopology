...
| ASMP-3 | The Weak-Verifier Soundness Frontier | Which superhuman work can a weaker, noisy semantic verifier judge soundly? | **Conditional obstruction; major revision before external review.** The [registered correlated-noise result](asmp3_weak_verifier_frontier/RESULT_v0_1.md) and [selected-atom barrier](asmp3_weak_verifier_frontier/SELECTION_BARRIER_RESULT_v0_2.md) delimit aggregate amplification. The [v0.3 `Refute`-interface result](asmp3_resolution_boundary_v0_3/NEGATIVE_RESOLUTION_CANDIDATE_v0_3.md) proves the exact parity-channel gap `(3/5)^log2(T) -> 0` under a stipulated binding parity-only grammar. Simulated ultra review accepted the probability core but found a material protocol-quantifier and complete-game-specification gap: under an existential richer-encoding reading, a bit-vector disagreement protocol restores constant gap `3/5`. Further parity-grid enumeration cannot decide that scope issue. |
