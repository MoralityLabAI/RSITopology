...
| ASMP-3 | The Weak-Verifier Soundness Frontier | Which superhuman work can a weaker, noisy semantic verifier judge soundly? | **Literal v0.1 iff internally refuted exactly in both directions; repaired online subclass characterized; unrestricted classification and external acceptance remain open.** The [v2.17 consolidated disposition](asmp3_consolidated_resource_scope_v2_17/RESOURCE_SCOPE_DISPOSITION_v2_17.md) binds 15 independently checked evidence packages and 344 sealed manifest members. V0.7 refutes frozen-interface sufficiency with optimal gap `(3/5)^d -> 0`; v2.5 refutes literal necessity for a nonbinding decidable `Refute`. The v2.10 one-shot extractor and v2.11 black-box minimality characterize the declared six-clause online contract, while the [v2.13 path-risk theorem](asmp3_correlated_path_risk_v2_13/CORRELATED_PATH_RISK_THEOREM_v2_13.md) gives `alpha >= max(0,1-s-delta_path)` for arbitrary dependence with controlled selected-path risk. The [v2.15 interactive theorem](asmp3_interactive_covering_frontier_v2_15/INTERACTIVE_COVERING_THEOREM_v2_15.md) proves exact marker value `min(1,Kq/N)`, and [v2.16](asmp3_bounded_soundness_frontier_v2_16/BOUNDED_SOUNDNESS_THEOREM_v2_16.md) extends it to exact completeness `s+(1-s)min(1,Kq/N)`. The full regression is 434 passing tests across 38 packages plus 34/34 standalone checkers. The [harness stop certificate](asmp3_consolidated_resource_scope_v2_17/HARNESS_STOP_CERTIFICATE_v2_17.md) permits resumption only for a normative interface decision, a newly registered non-black-box/cross-task class, or attributable external review (currently 0/2). |
| ASMP-4 | Two-Port Evaluator-Relative Confinement | What separate read and write information rates are necessary and sufficient for evaluator-relative control? | **Partial structural classification with an exact registration fork; canonical specification choice required.** The [metric-robust bilateral theorem](asmp4_metric_robust_collapse_v0_3/RESULT.md) proves a diagonal-quadrant region for every shared relabel-invariant transcript-tree cost when the serial code class is closed under both computation normal forms. The [v0.6 registration-fork theorem](asmp4_registration_fork_v0_6/RESULT.md) holds the plant, evaluator, authority, channels, and canonical metric fixed and proves that a computed sensor has corner `(1,1)` while a forced raw sensor has corner `(2,1)`; only upstream computation closure changes. Its full-reset theorem gives corner `(log2 kappa,log2 m)` for every sensor-partition grammar, while its fixed-transducer constrained-graph theorem gives `(h_f,h_g)` from finite subset-observer spectral radii. Its adaptive constrained-grammar theorem gives an exact finite Bellman recurrence and, by a credited prescribed-initial-state entropy-game reduction, the exact stationary-policy corner `(log2 rho_I,h_g)` for every finite registered graph; a strongly connected, aperiodic fixture has adaptive corner `(log2 phi,log2 phi)` versus best fixed corner `(1,log2 phi)`, and two independent censuses agree on all 120,050 three-mode two-partition cases at horizon four. The [v0.7 relational-action theorem](asmp4_relational_action_frontier_v0_7/RESULT.md) closes the nonunique-safe-action seam: arbitrary public-history selection between two registered sensors has the exact nonrectangular wedge joining `(log2 3,log2 3)` to `(2,1)`, proved for all horizons by a concave multiplicity moment. On that same plant the fully computed and forced-raw registries give distinct rectangles, producing three exact regions with only sensor registration changed. Its two independent harnesses exhaust 2,800 safe-action relations through four modes and 84,672 adaptive trees at horizon three. The golden-mean fixed-transducer fixture has forced/computed corners `(log2 phi,0)` and `(0,0)`. Exact rational realizations have unstable evaluator-normal multiplier `3/2`, zero tangent reset derivative, and nonzero normal-control derivatives. Central and import-independent parsers of the canonical source find all eight relevant architecture clauses but no clause selecting a sensor-computation closure, while the exact completions satisfy those explicit obligations. Thus the canonical phrase “registered causal code” must fix its sensor grammar before a broader region is well-posed. The [v0.4 heterogeneous-cost boundary](asmp4_heterogeneous_port_costs_v0_4/RESULT.md) and [v0.5 adaptive-history successor](asmp4_adaptive_history_collapse_v0_5/RESULT.md) remain exact boundary results: v0.5 gives the Bellman recursion and guarded rectangles for stationary ergodic public predictors, including a two-sided rational Shapley certificate. The [v0.2 theorem](asmp4_serial_collapse_theorem_v0_2/RESULT.md) supplies exact finite corrections; the earlier [108-cell game](asmp4_two_port_game/RESULT.md) remains a frozen memoryless-grammar result. |
...
prefix has length four. The relational successor exhausts all 2,800 nonempty
three-action relations through four modes, proves four modes minimal for a
strict local tradeoff, and independently reproduces all 84,672 horizon-three
adaptive candidates. Its all-horizon concave-moment converse gives a third,
nonrectangular exact region on the same plant. A general zero-error
support-derandomization theorem is independently stress-tested on 53,108
sensor kernels, including 950 genuinely randomized feasible supports, with
zero failures. The integrated ASMP-4 chain passes 98 tests; v0.6 retains 21
gates, and v0.7 passes ten central plus ten import-independent gates.

**ASMP-4 v0.8 stochastic addendum.** The canonical source permits
state-independent shared randomness and universally quantifies disturbances
but contains no clause ordering probability and disturbance. V0.8 proves that
per-disturbance almost-sure safety collapses to a common safe seed for
countable disturbance alphabets, yet separates on the smooth uncountable
diagonal `x_next=(u-w)^2`: the zero-message randomized code is safe almost
surely for every fixed disturbance path, while no seed is uniformly or
support-zero-error safe. Central and independent harnesses reproduce 484,524
finite-grid seed/disturbance pairs, 60 exact phase-map cells, and the two
noncommuting alphabet/horizon limits. The integrated chain now passes 112
tests; v0.8 passes eight central and seven import-independent gates.

**ASMP-4 v0.9 completion-atlas disposition.** The
[sealed completion atlas](asmp4_completion_atlas_v0_9/RESULT.md) extracts all
five canonical completion requirements and maps exact conditional evidence to
each, but proves canonical completion for `0/5`.  Two independent normative
dimensions remain absent: the registered sensor/computation class and the
probability/disturbance order.  On fixed plants, three compatible sensor
completions give three distinct regions and three compatible stochastic
completions give two distinct feasibility outcomes.  Because the canonical
clauses do not select among them, the target is semantically underdetermined.
The source plus seven predecessor claims are SHA-256 sealed; central and
import-independent builders reproduce the atlas, 112 predecessor tests, and
the four-condition reopening contract.  The integrated chain passes 127 tests,
while v0.9 passes eleven central and eight independent gates.  This supports
