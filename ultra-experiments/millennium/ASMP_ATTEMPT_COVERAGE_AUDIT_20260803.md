...
| 4 | ASMP-11 real-model/obfuscation access ladder | Existing evidence is transparent Boolean parity, not learned conditional defection. | **Completed current-host feasibility attempt, not a scientific result.** The v0.3 safety preflight found no inspected execution path establishing the required simultaneous hard caps of 2,048 MiB RAM, 50% CPU, and 50 MiB/s I/O, so it stopped before model loading or training; see [`REAL_MODEL_ACCESS_PREFLIGHT_v0_3.md`](asmp11_access_ladder/REAL_MODEL_ACCESS_PREFLIGHT_v0_3.md). Under this audit's weak criterion, that bounded failed execution attempt ends untouched status for this queue row. It observed no model behavior or access-ladder contrast, leaves the scientific successor open, and does not resolve ASMP-11. |
| 5 | ASMP-12 mixed strategies or equilibrium-selection dynamics | The current correspondence is finite and pure/constructible only. | **Completed selection-dynamic attempt.** Source-frozen v0.4 computed the registered finite initialized alternating-inertial minimum-cost best-response selector over 12 complete graphs and 24 trajectories and passed import-independent exact replay; see [`RESULT_v0_4.md`](asmp12_program_equilibrium/artifacts_v0_4_inertial_selector/RESULT_v0_4.md). This makes the selection-dynamic branch no longer untouched under the audit criterion, but mixed/correlated equilibria and general selection dynamics remain outside scope, and ASMP-12 remains unresolved. |
| 6 | ASMP-5 dynamic trusted-anchor replacement, learned-checker error, or infinite distinct-state progress | V0.3 keeps the trusted root fixed, although it already tests successor-checker replacement and a static root-permission mutation; arbitrary time comes from a repeated two-cycle. | **Completed attempt with an independent-verifier resource stop.** Source-frozen v0.4 compiled all 54 exact rows for a dynamically replaced replicated noisy anchor and recorded the finite formulas and fresh-replacement limit, but its frozen import-independent verifier exceeded the wall budget and wrote no verification artifact; see [`RESULT_v0_4.md`](asmp5_verifier_drift/artifacts_v0_4_dynamic_noisy_anchor/RESULT_v0_4.md). The task result exists, measurement reliability stopped, and claim support is not established. This is attempt coverage only--not a verified result, evidence about neural learned checkers or infinite distinct states, or a resolution of ASMP-5. |
| 7 | ASMP-4 full source-faithful capacity/support theorem beyond the existing finite boundary audit | The local nonfrozen v2 work already contains an exact eight-row source-factorization boundary, but no full capacity or support theorem. | **Completed negative-feasibility attempt for one proposed reopening design.** The self-contained v0.37 review argues that the proposed `128 x 8,192` Boolean registry census is not exhaustively derived from the canonical source and therefore cannot establish the claimed source-faithful capacity region or registry-independence theorem; see [`RESULT.md`](asmp4_registry_reopening_review_v0_37/RESULT.md). It preserves the v0.36 stop and does not use or promote the dirty v2/v2.1 drafts. The broader capacity/support theorem remains open, and ASMP-4 remains unresolved. |

Under the deliberately weak audit criterion, rows 4--7 now each contain a
bounded attempt or negative-feasibility attempt, so no listed successor
obligation remains untouched. This is an attempt-coverage classification only,
not mathematical resolution.
