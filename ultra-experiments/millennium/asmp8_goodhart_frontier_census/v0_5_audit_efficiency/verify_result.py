"""Independent replay verifier for ASMP-8 v0.5."""
...
def verify(artifact_dir: Path) -> dict[str, Any]:
    protocol_path = HERE / "protocol_v0_5.json"
    registration_path = HERE / "registration_v0_5.json"
    protocol = json.loads(protocol_path.read_text(encoding="utf-8"))
    registration = json.loads(registration_path.read_text(encoding="utf-8"))
    result = json.loads((artifact_dir / "result_v0_5.json").read_text(encoding="utf-8"))
    receipt = json.loads((artifact_dir / "receipt_v0_5.json").read_text(encoding="utf-8"))
    replay, conditions, policies, cdf = run_efficiency(protocol)
    checks = {
        ...
        "claim_boundary_present": result["claim_boundary"] in (
            artifact_dir / "RESULT_v0_5.md"
        ).read_text(encoding="utf-8"),
    }
    passed = all(checks.values())
    return {
        "schema_version": "asmp8_audit_efficiency_verification_v0_5",
...
def parse_args(argv: Iterable[str] | None = None):
    ...
    parser.add_argument("--artifact-dir", type=Path, default=HERE / "artifacts_v0_5")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    output = args.output.resolve() if args.output else artifact_dir / "verification_v0_5.json"
