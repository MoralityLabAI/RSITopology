"""Write-once runner for the source-only ASMP-12 inertial selector v0.4."""
...
def require_isolated_safe_path() -> None:
    """Reject launches whose import path can be shadowed by the source directory."""
...
def preflight_live_source_inventory() -> None:
    """Require exactly eight ordinary live source files before any local import."""
...
def bootstrap_source_snapshot(
    ...
    """Bind all live bytes to one ordinary ancestral commit and return a snapshot."""
...
def load_primary_module(source_bytes: bytes) -> ModuleType:
    """Compile only the already-bound snapshot, never a path-based local import."""
    source_path = HERE / "inertial_selector.py"
    module = ModuleType("_asmp12_v04_frozen_primary")
...
def validate_output_path(path: Path) -> Path:
    ...
    frozen_destination = Path(
        os.path.abspath(os.fspath(ARTIFACT_DIRECTORY / "result_v0_4.json"))
    )
    if destination != frozen_destination:
        raise SystemExit("refusing output outside the frozen result artifact path")
    try:
        destination.lstat()
...
def main() -> None:
    ...
    primary = load_primary_module(source_snapshot["inertial_selector.py"])
    ...
    try:
        primary.write_once_json(
            output_path, result, max_bytes=manifest["budget"]["max_artifact_bytes"]
        )
