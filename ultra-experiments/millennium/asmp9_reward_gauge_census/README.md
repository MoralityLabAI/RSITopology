...
The unregistered
[Gaussian quotient v0.70](gaussian_quotient_v0_70/THEOREM_DRAFT_v0_70.md)
then supplies one matching stochastic-complexity cell. For known-variance
independent Gaussian query rows, information positive-definiteness is the
exact access condition, `trace(M^-1)` is the exact minimax quotient MSE, and
`c^T M^-1 c` is the exact MSE for a declared policy margin. The exact integer
...
identification of the full-kernel object tier. The
[general finite-menu theorem](general_incomplete_menu_v0_56/RESULT_v0_56.md)
proves the same sharp tier-ambiguity boundary for every finite universe with
