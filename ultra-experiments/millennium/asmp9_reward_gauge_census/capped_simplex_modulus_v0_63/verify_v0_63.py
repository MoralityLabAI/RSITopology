"""Write-once prospective verifier for ASMP-9 v0.63."""
...
from primary_verifier_v0_63 import run_primary


HERE = Path(__file__).resolve().parent
REPO = HERE.parents[3]
REGISTRATION = HERE / "verification_registration_v0_63.json"
CELLS = HERE / "verification_cells_v0_63.json"
OUTPUT = HERE / "VERIFY_RESULT_v0_63.json"
REPLAY = HERE / "independent_replay_v0_63.py"
...
def replay_is_import_independent() -> bool:
    ...
    forbidden = {
        "capped_simplex_modulus",
        "primary_verifier_v0_63",
    }
    return not any(
...
def main() -> None:
    ...
    gates = {"H0": not mismatches and ancestor}

    tests = run_tests(registration)
    expected = f"{registration['expected_test_count']} passed"
    gates["T0"] = tests.returncode == 0 and expected in tests.stdout
    ...
    result = {
        ...
        "schema": "asmp9-v0.63-verification-result-v1",
        "source_commit": registration["source_commit"],
        "source_mismatches": mismatches,
        "source_was_ancestor": ancestor,
        "status": status,
        "tests": {
