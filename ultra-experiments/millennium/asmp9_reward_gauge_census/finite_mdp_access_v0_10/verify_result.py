...
def main() -> None:
    ...
    result = json.loads(
        (artifact_dir / "result_v0_10.json").read_text(encoding="utf-8")
    )
    receipt = json.loads(
        (artifact_dir / "receipt_v0_10.json").read_text(encoding="utf-8")
    )
    protocol = json.loads(
    ...
    checks = {
        ...
        == {"result_v0_10.json", "RESULT_v0_10.md"},
        "protocol_identity": (
            result["protocol_id"]
            == receipt["protocol_id"]
