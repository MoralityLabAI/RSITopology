...
if registration["status"] != "registered_prereveal":
    raise SystemExit("registration is not prereveal")
if registration["outcomes_read"] is not False:
    raise SystemExit("registration records prior outcome access")
if registration["phase"] != authorization["phase"]:
    raise SystemExit("phase mismatch")
caps = authorization["resource_caps"]
if caps != registration["resource_contract"]:
    raise SystemExit("resource caps differ from registration")
if int(caps["swap_bytes"]) != 0 or int(caps["batch_size"]) != 1:
    raise SystemExit("zero-swap singleton contract changed")
...
if (( RUNNER_STATUS == 0 )); then
  ...
  systemd-run \
    ...
    -p "RuntimeMaxSec=${TIMEOUT_SECONDS}" \
    "${RUNNER_PYTHON}" \
    "${REPO_ROOT}/ultra-experiments/millennium/asmp9_reward_gauge_census/context_quotient_response_v0_68/analyze_v068.py" \
