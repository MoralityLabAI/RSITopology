"""Independent deterministic replay for ASMP-9 v0.44."""
...
from run_confirmation_v0_44 import (  # noqa: E402
    PROTOCOL_ID,
    VERSION,
...
def verify(registration_path: Path, result_path: Path) -> dict:
    ...
    checks = {
        ...
        "scientific_payload": replay_scientific == result["scientific"],
        "scientific_gates": all(gate_matches.values()),
    }
    status_mapping = result["status"] == total_status(result["gates"])
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=HERE / "VERIFY_RESULT_v0_44.json",
    )
