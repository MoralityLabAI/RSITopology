...
from conditional_fiber import conditional_weights
...
def direct_zero_balance_fiber(
    cycle_length: int, trials_per_edge: int
) -> list[tuple[int, ...]]:
    if cycle_length < 2 or trials_per_edge < 0:
        raise ValueError("invalid cycle or trial count")
    return [
        (z,) * cycle_length for z in range(trials_per_edge + 1)
    ]


def run_power_calibration_streaming_direct(
    spec: dict[str, Any],
) -> dict[str, Any]:
    ...
    for cell in spec["cells"]:
        ...
        zero_fiber = direct_zero_balance_fiber(k, formula_trials)
    ...
    return {
        ...
        "exact_representation": (
            "streamed_unreduced_integer_ratios_with_direct_cycle_fiber"
        ),
    }
