...
def main() -> None:
    ...
    result = json.loads(
        (artifact_dir / "result_v0_13_1.json").read_text(encoding="utf-8")
    )
    receipt = json.loads(
        (artifact_dir / "receipt_v0_13_1.json").read_text(encoding="utf-8")
    )
    ...
    checks = {
        ...
        == {"result_v0_13_1.json", "RESULT_v0_13_1.md"},
        "protocol_identity": (
            result["protocol_id"]
            == receipt["protocol_id"]
        ...
        "scientific_specification_unchanged": (
            result["scientific_specification_unchanged"]
            and amendment["scientific_specification_unchanged"]
        ),
        "binding_checks": all(result["binding_checks"].values()),
        "all_gates_pass": all(result["gates"].values()),
        "verdict": result["verdict"] == amendment["success_verdict"],
    }
