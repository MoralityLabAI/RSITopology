# ASMP-9 contextual scalar-gluing protocol v0.11.1

## Status

Prospective resource-repair protocol. The registered v0.11 attempt exceeded
its 360-second wall-time cap and produced no scientific artifact. This version
changes only the tuple-census implementation by precomputing exact graph ranks
and components. It does not change any scientific cell, seed, status, gate,
threshold, or resource limit.
...
Before registration, the optimized census must equal the literal exact census
on all burned cells:

- two items, three contexts;
- three items, two contexts;
- three items, three contexts; and
- all 4,096 four-item/two-context graph pairs.

The optimized code still computes exact rational incidence ranks for all 64
possible four-item graphs. Tuple traversal only reuses those ranks; it does
not replace rank with the component formula being tested.

## Gates

The ten scientific and resource gates are unchanged from v0.11:
...
```text
finite_contextual_scalar_gluing_geometry_verified_v0_11_1
```
